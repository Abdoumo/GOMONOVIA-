import Header from "@/components/Header";
import Footer from "@/components/Footer";

export default function Science() {
  return (
    <div className="min-h-screen bg-white">
      <Header />

      {/* Hero Section */}
      <section className="bg-gradient-to-br from-sage-50 to-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-5xl md:text-6xl font-bold text-sage-900 mb-6">
            Science &amp; Recherche
          </h1>
          <p className="text-xl text-sage-600 max-w-2xl">
            Découvrez les fondements scientifiques derrière GOMONOVIA et notre approche de validation rigoureuse.
          </p>
        </div>
      </section>

      {/* Main Content */}
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Scientific Foundations */}
          <div className="mb-20">
            <h2 className="text-4xl font-bold text-sage-900 mb-8">Fondements Scientifiques</h2>

            <div className="bg-sage-50 rounded-2xl p-12 mb-8">
              <p className="text-sage-700 text-lg leading-relaxed mb-6">
                La régulation de la satiété repose sur des interactions complexes entre le système digestif, les signaux hormonaux et le système nerveux central. La littérature scientifique souligne le rôle des fibres alimentaires et de certains composés d'origine végétale dans ces processus.
              </p>

              <div className="grid md:grid-cols-3 gap-6 mt-8">
                <div className="bg-white rounded-lg p-6 border-l-4 border-sage-600">
                  <h3 className="font-bold text-sage-900 mb-3">Système Digestif</h3>
                  <p className="text-sage-700 text-sm">
                    Interactions complexes impliquant l'absorption et la digestion
                  </p>
                </div>
                <div className="bg-white rounded-lg p-6 border-l-4 border-sage-600">
                  <h3 className="font-bold text-sage-900 mb-3">Signaux Hormonaux</h3>
                  <p className="text-sage-700 text-sm">
                    Régulation via hormones de satiété et appétence
                  </p>
                </div>
                <div className="bg-white rounded-lg p-6 border-l-4 border-sage-600">
                  <h3 className="font-bold text-sage-900 mb-3">Système Nerveux</h3>
                  <p className="text-sage-700 text-sm">
                    Communication centrale avec les signaux périphériques
                  </p>
                </div>
              </div>
            </div>

            <h3 className="text-2xl font-bold text-sage-900 mb-6">Rôle des Ingrédients Naturels</h3>
            <div className="space-y-4">
              <div className="border-l-4 border-sage-600 pl-6 py-4">
                <h4 className="font-bold text-sage-900 mb-2">Fibres Alimentaires</h4>
                <p className="text-sage-700">
                  Les fibres jouent un rôle crucial dans la promotion de la satiété en ralentissant la digestion et en augmentant le volume du bol alimentaire.
                </p>
              </div>
              <div className="border-l-4 border-sage-600 pl-6 py-4">
                <h4 className="font-bold text-sage-900 mb-2">Extraits Végétaux</h4>
                <p className="text-sage-700">
                  Certains composés végétaux ont démontré des propriétés de modulation de l'appétit et de support métabolique dans la littérature scientifique.
                </p>
              </div>
              <div className="border-l-4 border-sage-600 pl-6 py-4">
                <h4 className="font-bold text-sage-900 mb-2">Approche Multimodale</h4>
                <p className="text-sage-700">
                  GOMONOVIA combine plusieurs ingrédients pour un effet synergique maximisant les bénéfices naturels.
                </p>
              </div>
            </div>
          </div>

          {/* Research Approach */}
          <div className="mb-20">
            <h2 className="text-4xl font-bold text-sage-900 mb-8">Orientation Recherche</h2>

            <p className="text-sage-700 text-lg leading-relaxed mb-12">
              GOMONOVIA adopte une démarche structurée incluant l'optimisation des formulations, l'évaluation de la sécurité et la préparation à de futures études cliniques.
            </p>

            <div className="space-y-6">
              <div className="bg-white border-2 border-sage-200 rounded-2xl p-8">
                <h3 className="text-2xl font-bold text-sage-900 mb-4">Phase 1 : Optimisation des Formulations</h3>
                <p className="text-sage-700 mb-4">
                  Nous sélectionnons et optimisons les ingrédients naturels en fonction des meilleures pratiques scientifiques et des données d'efficacité disponibles.
                </p>
                <ul className="space-y-2 text-sage-700">
                  <li className="flex items-start gap-3">
                    <span className="text-sage-600">•</span>
                    <span>Analyse comparative de sources naturelles</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="text-sage-600">•</span>
                    <span>Évaluation de la biodisponibilité</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="text-sage-600">•</span>
                    <span>Tests de stabilité et de potentiel</span>
                  </li>
                </ul>
              </div>

              <div className="bg-white border-2 border-sage-200 rounded-2xl p-8">
                <h3 className="text-2xl font-bold text-sage-900 mb-4">Phase 2 : Analyse Biochimique</h3>
                <p className="text-sage-700 mb-4">
                  Une compréhension profonde des mécanismes d'action au niveau moléculaire et cellulaire.
                </p>
                <ul className="space-y-2 text-sage-700">
                  <li className="flex items-start gap-3">
                    <span className="text-sage-600">•</span>
                    <span>Caractérisation des composés actifs</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="text-sage-600">•</span>
                    <span>Études de mécanisme d'action</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="text-sage-600">•</span>
                    <span>Évaluation de la synergie des ingrédients</span>
                  </li>
                </ul>
              </div>
            </div>
          </div>

          {/* Validation Roadmap */}
          <div>
            <h2 className="text-4xl font-bold text-sage-900 mb-12 text-center">
              Feuille de Route de Validation
            </h2>

            <div className="space-y-6">
              {[
                {
                  step: 1,
                  title: "Tests en Laboratoire",
                  description: "Évaluation complète de la sécurité, de la stabilité et de la qualité du produit dans un environnement contrôlé.",
                  status: "completed"
                },
                {
                  step: 2,
                  title: "Études Précliniques",
                  description: "Recherche en modèles cellulaires et animaux pour valider les mécanismes d'action et l'efficacité potentielle.",
                  status: "in-progress"
                },
                {
                  step: 3,
                  title: "Évaluations de Sécurité",
                  description: "Tests complets pour assurer l'innocuité du produit, y compris les études de toxicologie et de tolérance.",
                  status: "pending"
                },
                {
                  step: 4,
                  title: "Essais Cliniques Futurs",
                  description: "Études chez l'humain pour démontrer l'efficacité et la sécurité dans des conditions réelles d'utilisation.",
                  status: "planned"
                }
              ].map((item) => (
                <div key={item.step} className="flex gap-6">
                  <div className="flex flex-col items-center">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg ${
                      item.status === 'completed' ? 'bg-sage-600 text-white' :
                      item.status === 'in-progress' ? 'bg-sage-300 text-sage-900' :
                      'bg-sage-100 text-sage-600'
                    }`}>
                      {item.status === 'completed' ? '✓' : item.step}
                    </div>
                    {item.step !== 4 && (
                      <div className="w-1 h-16 bg-sage-200 mt-2" />
                    )}
                  </div>
                  <div className="flex-1 pb-8">
                    <h3 className="text-xl font-bold text-sage-900 mb-2">{item.title}</h3>
                    <p className="text-sage-700">{item.description}</p>
                    <span className={`inline-block mt-3 text-sm font-semibold px-3 py-1 rounded-full ${
                      item.status === 'completed' ? 'bg-green-100 text-green-800' :
                      item.status === 'in-progress' ? 'bg-blue-100 text-blue-800' :
                      'bg-gray-100 text-gray-800'
                    }`}>
                      {item.status === 'completed' ? 'Complété' :
                       item.status === 'in-progress' ? 'En cours' :
                       'Planifié'}
                    </span>
                  </div>
                </div>
              ))}
            </div>
          </div>
        </div>
      </section>

      <Footer />
    </div>
  );
}
